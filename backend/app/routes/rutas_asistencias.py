...
from app.schemas.esquema_asistencias import (
    ...
    GenerarQRAistenciaRequest,
    GenerarQRAistenciaResponse,
    RegistrarAsistenciaQRRequest,
...
@router.post("/qr/generate", response_model=GenerarQRAistenciaResponse)
def generar_qr(request: GenerarQRAistenciaRequest, token: str, db: Session = Depends(get_db)):
    current_user = get_current_user(token, db)
    return generar_qr_asistencia(request.activity_id, current_user, db)


@router.post("/qr/register", response_model=AsistenciaRespuesta, status_code=201)
def registrar_por_qr(request: RegistrarAsistenciaQRRequest, token: str, db: Session = Depends(get_db)):
    current_user = get_current_user(token, db)
    return registrar_asistencia_por_qr(request.token, current_user, db)
